...
def mlmd_sampling(molecule_object, ase_calculator, dt, maxt, Escut, Fscut, Ncheck, Tamp, Tper, Tsrt, Tend, Ramp, Rper,
    ...
    for i in range( int( np.ceil((1000*maxt)/(dt*Ncheck)) ) + 1 ):
        ...
        if (i*Ncheck*dt/1000) >= min_time:
            if Ecrit or Fcrit or Fmcrit or distcrit:
                ...
                ###RAM: We want to find just the first failed geometry but for speed tests let it keep running
                ###RAM: Find when failed was still false
                if not failed: 
                    meta_dict_fail = {"Es": Es,
                                      "Fs": Fs,
                                      "Fsmax" : Fsmax,
                                      "distmin": dist_min,
                                      "temps" : temps,
                                      ...
                                      "Fmcrit" : Fmcrit,
                                      "distcrit": distcrit,
                                      "chemical_symbols" : ase_atoms.copy().get_chemical_symbols(),
                                      "positions" : ase_atoms.copy().get_positions(wrap=True),
                                      "cell" : ase_atoms.copy().get_cell()
                                     }
                failed = True
                ###RAM: For CAAR tests, no longer breaking on fail
                #break
        ...
        if i % step_count == 0:
            time_current_step = time.time()
            if i == 0: 
                time_previous_step = sim_start_time
            else:
                time_previous_step = sim_start_time + time_dict["realtime_step_"+str(i-step_count)]
            time_dict["realtime_step_"+str(i)] = time_current_step - sim_start_time
            time_dict["delta_realtime_step_"+str(i)] = time_current_step - time_previous_step

    sim_time_end = time.time()
    sim_time = sim_time_end - sim_start_time

    meta_dict = {"realtime_simulation" : sim_time,
                    "Es"    : Es,
                    "Fs"    : Fs,
                    "Fsmax" : Fsmax,
                    "distmin": dist_min,
    ...
    ###RAM: 9/24 We want to override meta_dict if found a failed geometry, with the first fail
    if failed:
        for key, val in meta_dict_fail.items():
            meta_dict[key] = val
    ...
    ###RAM: 9/4/26: Modified to save ase_atoms because we want final structure
...
def simple_mlmd_sampling_task(molecule_object, sampler_config, model_path, current_model_id, gpus_per_node):
    ...
    #os.environ["ROCR_VISIBLE_DEVICES"] = str(int(os.environ.get('PARSL_WORKER_RANK'))%gpus_per_node) ###RAM: Does not properly assign GPUs with this environment variable specified
    feed_parameters = {}
